...
// create a component
class EasySlideScreen extends Component {
    constructor(props){
        super(props);
        this.state = {
            data: [],
   ...
   componentWillMount() {
        axios.get(CONFIG.API_URL + "/skill/2/2")
        .then((response) => {
            this.props.dispatch({
                type: "GET_DATA_EASY_SLIDE",
                data: response.data,
            })
        })
          .catch((error) => {
                        ...
                        renderItem={({ item }) => 
                            <View>
                                ...
                                <TouchableOpacity  style={{paddingTop:5,paddingBottom:5,flexDirection:'row'}} 
                                 >
                                    <Image source={require('../img/wheel2.png')} style={styles.iconlist}/>
...
function mapStateToProps(state) {
    return {
    EasySlide: state.EasySlide
    }
}
//make this component available to the app
export default connect(mapStateToProps)(EasySlideScreen);
